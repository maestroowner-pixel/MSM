...
// Desktop (Windows/macOS): no expo-av → play via the native SoundModule instead.
const onDesktop = onWindows || onMacOS;

// Audio mode: play even when the device is on silent (iOS), duck others on Android.
// (Skipped on desktop, where expo-av is unavailable and the native SoundModule is used.)
if (!onDesktop) {
...
/**
 * Play a bundled sound. iOS/Android use expo-av (by asset require). Desktop
 * (Windows/macOS) has no expo-av, so it calls the native SoundModule, which plays
 * the bundled <name>.mp3 by name (Windows: Bundle\assets\sounds\; macOS:
 * Resources/Sounds/, see SoundModule.m). See WINDOWS.md / MACOS.md.
 */
