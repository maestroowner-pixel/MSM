...
import { macSaveFile, macOpenFile } from './MacFileManager';

export const onWindows = Platform.OS === 'windows';
export const onMacOS = Platform.OS === 'macos';
/** Desktop (Windows/macOS): no expo-sharing/file-system → use a native Save/Open panel. */
export const onDesktop = onWindows || onMacOS;

/** Deliver generated content to the user (share sheet on mobile, native Save dialog on desktop). */
export async function deliverFile(
  ...
  if (onMacOS) {
    await macSaveFile(fileName, data, isBase64);
    return;
  }
...
/** Desktop-only file pick → base64 content. Returns null off-desktop or on cancel. */
export async function pickFileBase64(
  ...
  if (onMacOS) {
    const r = await macOpenFile(extensions);
    return r ? { name: r.name, base64: r.content } : null;
  }
  return null;
}
