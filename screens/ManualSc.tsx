      ...
      {content.sections.map((s, i) => {
        ...
        return (
          <View key={s.title} style={styles.card}>
            ...
            {expanded ? (
              <View style={[styles.body, s.octopus && Platform.OS === 'macos' && styles.bodyOctopusMac]}>
                {s.octopus ? (
                  // react-native-macos ignores resizeMode="contain", so the full-bleed
                  // background stretches the (near-square) octopus across the wide box.
                  // On macOS, center it at a fixed aspect-correct size as a watermark.
                  Platform.OS === 'macos' ? (
                    <View style={styles.octopusWrapMac} pointerEvents="none">
                      <Image source={require('../assets/octopus.png')} style={styles.octopusMac} />
                    </View>
                  ) : (
...
const makeStyles = (COLORS: Palette) => StyleSheet.create({
  ...
  // macOS watermark: centered, aspect-correct (octopus.png is 645×629), not stretched.
  octopusWrapMac: { position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, alignItems: 'center', justifyContent: 'center' },
  octopusMac: { width: 168, height: 164, opacity: 0.32 },
  // Ensure the About card is tall enough to show the full octopus watermark (164 + padding).
  bodyOctopusMac: { minHeight: 188 },
