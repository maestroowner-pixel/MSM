...
export default function ItemDetailSc() {
  ...
  const chooseReplace = (att: Attachment) => {
    Alert.alert('Replace file', 'Choose a source', [
      // macOS has no camera — omit that source there.
      ...(Platform.OS === 'macos' ? [] : [{ text: 'Camera', onPress: replaceWith(att, pickFromCamera) }]),
  ...
  const attachmentMenu = (att: Attachment) => {
    Alert.alert(att.name ?? 'File', undefined, [
      ...
      { text: 'Delete', style: 'destructive' as const, onPress: () => removeAttachment(att) },
      { text: 'Cancel', style: 'cancel' as const },
    ]);
  };
  ...
  return (
    <LinearGradient colors={COLORS.bgGradient} style={{ flex: 1 }}>
      <SafeAreaView style={{ flex: 1 }} edges={['top', 'left', 'right']}>
        ...
        <PlatformModal visible={!!renaming} transparent animationType="fade" onRequestClose={() => setRenaming(null)}>
        ...
        <PlatformModal visible={certPicker} transparent animationType="fade" onRequestClose={() => setCertPicker(false)}>
          <View style={styles.rnBackdrop}>
