...
import { deliverFile, onWindows, onMacOS, onDesktop } from '../utils/fileShare';
import { macHtmlToPdfBase64 } from '../utils/MacFileManager';
...
export async function exportPdf(
  ...
  // macOS: expo-print is mocked → render the PDF natively (WKWebView) and save it
  // through the native Save panel. (See MacFileManager / FileManagerModule.m.)
  if (onMacOS) {
    const pdfB64 = await macHtmlToPdfBase64(html);
    if (!pdfB64) throw new Error('Could not render the PDF.');
    await deliverFile(`MSM_report_${fileDateStamp()}.pdf`, pdfB64, true, 'application/pdf');
    return;
  }
...
export async function printReport(
  ...
  if (onDesktop) throw new Error('Printing is not available on desktop — export a PDF instead.');
  const groups = selectedCategories(byCategory, only);
...
export async function exportZip(
  ...
  if (onDesktop) throw new Error('ZIP export is not available on desktop — use PDF, XLSX, or a .msm backup.');
  const groups = selectedCategories(byCategory, only);
  if (!groups.length) throw new Error('No items to export.');
